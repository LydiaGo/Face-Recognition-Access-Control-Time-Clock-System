...
class CameraApp:
    ...
    def capture_photo(self):
        ...
        if ret:
            ...
            # 將圖像轉換為NumPy陣列並提取特徵嵌入
            image_array = np.array(frame)
            face_embedding = self.get_face_embedding(image_array)
            
            self.record_to_excel(employee_id, employee_name, jpg_path, npy_path, face_embedding)
    ...
    def record_to_excel(self, employee_id, employee_name, jpg_path, npy_path, face_embedding):
        excel_file = "employee_records.xlsx"
        if not os.path.exists(excel_file):
            workbook = openpyxl.Workbook()
            sheet = workbook.active
            sheet.append(["員工編號", "員工姓名", "JPG檔案", "NPY檔案", "Face Embedding"])
        else:
            workbook = openpyxl.load_workbook(excel_file)
            sheet = workbook.active

        sheet.append([employee_id, employee_name, jpg_path, npy_path, str(face_embedding)])
