...
# 使用face_recognition獲取人臉嵌入的函數（判斷是否有人臉）
...
# 計算兩個人臉嵌入之間的相似度的函數（算現在畫面跟資料庫每個npy檔的相似度）
def cosine_similarity(embedding1, embedding2):
    return 1 - distance.cosine(embedding1, embedding2)

# 從面部嵌入列表中查找最相似面部的函數（抓相似度比例最高的回傳）
def find_most_similar(target_embedding, face_embeddings):
    ...
    most_similar_index = np.argmax(similarities) #抓相似度最高
...
def grab_face_and_save_embedding():
        ...
        ret, frame = cap.read()  #抓影像
        frame1 = frame.copy()
            # Convert the frame to RGB (face_recognition uses RGB)
        rgb_frame = cv2.cvtColor(frame1, cv2.COLOR_BGR2RGB) #改影像顯示底色方便辨識
        pil_img = Image.fromarray(frame1)
            # Get face locations using face_recognition
        face_locations = face_recognition.face_locations(rgb_frame) #抓人臉位置
            #print(rgb_frame)
        ...
        capture_tk = ImageTk.PhotoImage(image=frame2) #改影像格式以顯示
