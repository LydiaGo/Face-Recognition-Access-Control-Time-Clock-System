...
b_data = pd.DataFrame(columns=["number", "ID", "state" , "date", "time"])
try:
...
def grab_face_and_save_embedding():
        ...
        for face_location in face_locations:
            ...
            if b1.cget("state") == "active":
                ...
                b_data = b_data.append({"number":number, "ID": target_id, "state": work, "date": date1, "time": time1}, ignore_index=True)
